...
export default function Sidebar() {
  ...
  const navigate = useNavigate(); // Hook to navigate programmatically
    ...
    const handleUploadClick = () => {
      navigate('/uploaded'); // Redirect to /uploaded page
    };
  ...
  return (
    <div className="flex">
      ...
      <div
        ...
        className={`sidebar text-primaryText h-screen transition-width duration-300 ease-in-out relative`}
      >
      ...
      {/* Box with Inner Box */}
      <div className="flex-grow p-4 bg-thirdBg flex justify-center items-center max-h-fit">
      <div className="mt-40 p-4 shadow-md rounded-md" style={{ width: '596px', height: '367px' , backgroundColor: 'var(--excel-color)' }}>
        {/* Inner Box */}
        <div className="flex flex-col items-center justify-center p-8 border-dashed border-2  rounded-md" style={{ height: '250px' , backgroundColor: 'var(--excel-color)' }}>
          <img
            src={file}
            alt="Excel"
            className="w-12 h-12 mb-4"
          />
          <p id="drop" className="text-gray-500">Drop your excel sheet here or <span id="browse">browse</span></p>
        </div>
        {/* Upload Button */}
        <div className="flex justify-center mt-4">
          <button
            onClick={handleUploadClick}
            className="w-full flex items-center justify-center px-4 py-4 text-white rounded-lg hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-opacity-50" style={{backgroundColor: '#605BFF'}}
          >
